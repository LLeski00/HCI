...
const Review: FC<ReviewProps> = ({ review }) => {
  ...
  const snippet = review.text
    ? review.text.slice(0, snippetLengths) +
      (review.text.length > snippetLengths ? '...' : '')
    : '';
  ...
  useEffect(() => {
    if (user && review.reactions) {
      const userReaction = review.reactions.find((r) => r.userId === user.id);
    ...
    <div className={styles.reviewContainer}>
      ...
      <div className={styles.textContent}>
        {review.text && (
          <p className={styles.ratingText}>
            {isExpanded ? review.text : snippet}
          </p>
        )}
      ...
      <div className={styles.reactionContainer}>
        <div className={styles.reactionGroup}>
          ...
          {userReaction === Reaction.Like ? (
            <BiSolidLike onClick={() => handleReaction(Reaction.Like)} />
          ) : (
        ...
        <div className={styles.reactionGroup}>
          {userReaction === Reaction.Dislike ? (
            <BiSolidDislike onClick={() => handleReaction(Reaction.Dislike)} />
          ) : (
            <BiDislike onClick={() => handleReaction(Reaction.Dislike)} />
          )}
          <p>
            {userReaction === Reaction.Dislike
      ...
      {review.text && review.text.length <= snippetLengths ? (
