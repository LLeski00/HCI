...
const PlannerResults: FC<PlannerResultsProps> = ({ results }) => {
  ...
  if (!results.bestMatch || !results.cheapest || !results.closest)
    return (
      <p style={{ textAlign: 'center', margin: '0 0 30px 0', color: 'red' }}>
        No results found for the given form! (Budget too low!)
      </p>
    );

  const resultResort = (title: string, plannerResult: PlannerResultResort) => {
    return (
      <div className={styles.resultContainer}>
        ...
        <ResortCard
          ...
          user={user}
          isFavourite={favouriteIds?.includes(plannerResult.resort.id) ?? false}
        />
